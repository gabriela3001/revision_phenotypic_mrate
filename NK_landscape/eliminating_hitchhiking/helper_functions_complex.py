...
def population_breaking_association(genotypes_names, pop, initial_genotype):
    
    mut_mrate = pM_proportion(genotypes_names, pop, initial_genotype)
    
    length_genotypes = len(genotypes_names) // 2
    
    nonmut_pop = (pop[:length_genotypes]*(1-mut_mrate)) /np.sum(pop[:length_genotypes])
    mut_pop = (pop[length_genotypes:]*mut_mrate) /np.sum(pop[length_genotypes:])  
